...
public class RestfulController {
    ...
    public List<PlaylistInfoDTO> playListBox(){
        System.out.println("1 : ");
        return psvc.playList_bogi();
    }
    ...
    public ResponseEntity<Integer> increaseLike(@PathVariable int bNum) {
        boardRepository.incrementBLike(bNum); // 추천 수 증가
        BoardEntity boardEntity = boardRepository.findById(bNum).orElseThrow(); // 게시글 조회
    ...
    public ResponseEntity<Integer> increaseDislike(@PathVariable int bNum) {
        boardRepository.incrementBDislike(bNum); // 비추천 수 증가
        BoardEntity boardEntity = boardRepository.findById(bNum).orElseThrow(); // 게시글 조회
